...
try{
 ...
 const storage=await Promise.all([take(clients[2]),take(clients[3])]);assert.equal(storage.filter(Boolean).length,1,'only one player may take an overlapping warehouse quantity');const qty=(await clients[2].query('SELECT quantity FROM inventory_items WHERE id=$1',[itemId])).rows[0].quantity;assert.equal(Number(qty),3,'warehouse quantity must remain consistent after race');
