...
export const canUseVehicle=async(characterId:number,vehicleId:string):Promise<boolean>=>{
 const r=await sql<{ok:boolean}>`SELECT EXISTS(SELECT 1 FROM vehicles v WHERE v.id=${vehicleId}::bigint AND (v.owner_character_id=${characterId} OR EXISTS(SELECT 1 FROM vehicle_access a WHERE a.vehicle_id=v.id AND a.character_id=${characterId}))) AS ok`.execute(db());
 return Boolean(r.rows[0]?.ok);
};

export const listVehicleAccess=async(ownerCharacterId:number,vehicleId:string):Promise<VehicleAccessView[]>=>{
 const owner=await sql`SELECT id FROM vehicles WHERE id=${vehicleId}::bigint AND owner_character_id=${ownerCharacterId}`.execute(db());if(!owner.rows.length)throw new Error('VEHICLE_NOT_FOUND');
 const r=await sql<{character_id:number;first_name:string;last_name:string;access_level:'driver'|'manager';created_at:Date}>`SELECT a.character_id,c.first_name,c.last_name,a.access_level,a.created_at FROM vehicle_access a JOIN characters c ON c.id=a.character_id WHERE a.vehicle_id=${vehicleId}::bigint ORDER BY a.created_at`.execute(db());
...
export const listSharedVehicles=async(characterId:number):Promise<VehicleView[]>=>{
 ...
 return r.rows.map((x:any)=>({id:String(x.id),model:x.model,vin:x.vin,plate:x.plate,fuel:x.fuel,mileage:x.mileage,engineHealth:x.engine_health,bodyHealth:x.body_health,locked:x.locked,impounded:x.impounded,insuranceUntil:x.insurance_until?new Date(x.insurance_until).toISOString():null,spawned:false}));
};
