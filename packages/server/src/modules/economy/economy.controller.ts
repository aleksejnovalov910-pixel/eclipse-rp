import { ErrorCode, RpcEvent, SessionState, err, ok, type Result } from '@eclipse/shared';
...
interface TransferRequest extends AmountRequest {
  toCharacterId: number;
  description?: string;
}

const requirePlayingCharacter = (session: Session): Result<number> => {
  if (session.state !== SessionState.Playing || session.characterId === null) {
...
const mapEconomyError = (error: unknown): ReturnType<typeof err> => {
  ...
  switch (code) {
    ...
    case 'SAME_ACCOUNT':
      return err(ErrorCode.SameAccount);
...
export const registerEconomyModule = (): void => {
  ...
  onRpc<unknown, service.BalanceSnapshot>(RpcEvent.EconomyBalance, async (ctx) => {
    const limited = consume(ctx.session, 'economy:balance', readRule);
    if (limited) return limited;

    const character = requirePlayingCharacter(ctx.session);
    if (!character.ok) return character;

    try {
      return ok(await service.getBalance(character.data));
    } catch (error) {
      return mapEconomyError(error);
    }
  });

  onRpc<AmountRequest, service.BalanceSnapshot>(RpcEvent.EconomyDeposit, async (ctx, payload) => {
    const limited = consume(ctx.session, 'economy:deposit', moneyRule);
    if (limited) return limited;
  ...
  onRpc<AmountRequest, service.BalanceSnapshot>(RpcEvent.EconomyWithdraw, async (ctx, payload) => {
    const limited = consume(ctx.session, 'economy:withdraw', moneyRule);
    if (limited) return limited;
  ...
  onRpc<TransferRequest, service.BalanceSnapshot>(RpcEvent.EconomyTransfer, async (ctx, payload) => {
    const limited = consume(ctx.session, 'economy:transfer', transferRule);
    if (limited) return limited;

    const character = requirePlayingCharacter(ctx.session);
    if (!character.ok) return character;

    if (!Number.isInteger(payload?.toCharacterId) || payload.toCharacterId <= 0) {
      return err(ErrorCode.Validation, { field: 'toCharacterId' });
    }
