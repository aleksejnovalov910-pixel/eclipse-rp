...
const expireStale=async():Promise<void>=>{await sql`UPDATE marketplace_listings SET status='cancelled' WHERE status='active' AND expires_at<=NOW()`.execute(db());};
export const listActive=async(c:number):Promise<MarketplaceListingView[]>=>{await expireStale();const r=await sql<ListingRow>`SELECT * FROM marketplace_listings WHERE status='active' AND expires_at>NOW() ORDER BY created_at DESC LIMIT 200`.execute(db());return r.rows.map(x=>toView(x,c));};
export const listMine=async(c:number):Promise<MarketplaceListingView[]>=>{await expireStale();const r=await sql<ListingRow>`SELECT * FROM marketplace_listings WHERE seller_character_id=${c} ORDER BY created_at DESC LIMIT 100`.execute(db());return r.rows.map(x=>toView(x,c));};

const ownedObject=async(c:number,type:MarketplaceObjectType,id:string):Promise<string>=>{
 if(type==='vehicle'){const cars=await listOwnedVehicles(c);const car=cars.find(v=>v.id===id);if(!car)throw new Error('OBJECT_NOT_OWNED');if(car.spawned||car.impounded)throw new Error('OBJECT_BUSY');return `${car.model}${car.plate?` · ${car.plate}`:''}`;}
...
export const createListing=async(c:number,p:MarketplaceCreateRequest):Promise<MarketplaceListingView>=>{
 ...
 return db().transaction().execute(async trx=>{
   await sql`UPDATE marketplace_listings SET status='cancelled' WHERE object_type=${p.objectType} AND object_id=${p.objectId}::uuid AND status='active' AND expires_at<=NOW()`.execute(trx);
   try{const r=await sql<ListingRow>`INSERT INTO marketplace_listings(seller_character_id,object_type,object_id,title,price) VALUES(${c},${p.objectType},${p.objectId}::uuid,${title},${price}::numeric) RETURNING *`.execute(trx);return toView(r.rows[0]!,c);}catch(e:unknown){const code=typeof e==='object'&&e&&'code'in e?String((e as {code?:unknown}).code):'';if(code==='23505')throw new Error('ALREADY_LISTED');throw e;}
 });
};

export const cancelListing=async(c:number,id:string):Promise<MarketplaceListingView>=>db().transaction().execute(async trx=>{const r=await sql<ListingRow>`SELECT * FROM marketplace_listings WHERE id=${id}::uuid FOR UPDATE`.execute(trx);const l=r.rows[0];if(!l||l.seller_character_id!==c||l.status!=='active'||l.expires_at<=new Date())throw new Error('LISTING_NOT_ACTIVE');const u=await sql<ListingRow>`UPDATE marketplace_listings SET status='cancelled' WHERE id=${id}::uuid RETURNING *`.execute(trx);return toView(u.rows[0]!,c);});

export const buyListing=async(buyer:number,id:string):Promise<MarketplacePurchaseResult>=>{
 const pre=await sql<ListingRow>`SELECT * FROM marketplace_listings WHERE id=${id}::uuid AND status='active' AND expires_at>NOW()`.execute(db());const preListing=pre.rows[0];if(!preListing)throw new Error('LISTING_NOT_ACTIVE');if(preListing.object_type==='vehicle'){const cars=await listOwnedVehicles(preListing.seller_character_id);const car=cars.find(v=>v.id===preListing.object_id);if(!car||car.spawned||car.impounded)throw new Error('OBJECT_BUSY');}
