...
import FuelPanel from '../components/FuelPanel.vue';

type Tab = 'bank'|'inventory'|'jobs'|'quests'|'family'|'property'|'business'|'market'|'fuel'|'garage';
...
const vehicleAction = async (event: string, car: VehicleView): Promise<void> => {
  if(busy.value)return;busy.value=true;error.value='';
  try{
    const r=await rpc<VehicleActionView|VehicleView>(event,{vehicleId:car.id});
    if(!r.ok){fail(r.code);return;}
    await load();
  }finally{busy.value=false;}
};
  ...
  <div class="tablet-shell e-interactive">
    <section class="tablet">
      <aside class="sidebar">
        ...
        <button :class="{active:tab==='fuel'}" @click="tab='fuel'">АЗС</button>
        <button :class="{active:tab==='garage'}" @click="tab='garage'">Гараж</button>
        <button class="close" @click="close">Закрыть</button>
      </aside>
      <main class="main">
        <header>
          <div><small>ПЛАНШЕТ</small><h1>{{ {bank:'Финансы',inventory:'Инвентарь',jobs:'Работы',quests:'Задания',family:'Семья',property:'Недвижимость',business:'Бизнес',market:'V‑Market',fuel:'АЗС',garage:'Транспорт'}[tab] }}</h1></div>
        ...
        <FuelPanel v-else-if="tab==='fuel'" />

        <template v-else>
          <div class="vehicles">
            <article v-for="car in vehicles" :key="car.id">
              <div><strong>{{ car.model }}</strong><span>VIN {{ car.vin }} · {{ car.plate || 'без номера' }}</span><small>{{ car.spawned ? 'Машина в мире' : 'В гараже' }}</small></div>
              <div class="vehicle-stats"><span>Топливо {{ car.fuel }}%</span><span>{{ car.mileage }} км</span><span :class="{bad:car.impounded}">{{ car.impounded?'Штрафстоянка':car.locked?'Закрыт':'Открыт' }}</span></div>
              <div class="vehicle-actions">
                <button v-if="!car.spawned" :disabled="busy||car.impounded" @click="vehicleAction(RpcEvent.VehicleSpawn,car)">Доставить</button>
                <button v-else :disabled="busy" @click="vehicleAction(RpcEvent.VehicleStore,car)">В гараж</button>
                <button :disabled="busy||car.impounded" @click="vehicleAction(RpcEvent.VehicleToggleLock,car)">{{ car.locked?'Открыть':'Закрыть' }}</button>
              </div>
            </article>
            <p v-if="!vehicles.length" class="empty">Личного транспорта пока нет</p>
          </div>
        </template>
      </main>
    </section>
